...
python3 "$ROOT/src/steering/steering_run.py" \
    --fname "arc_challenge_experiment" \
    --cache_dir "$SCRATCH/mera-runs/" \
    --save_dir "$SCRATCH/mera-runs/steering_outputs/" \
    --model_names "swiss-ai/Apertus-8B-2509" \
    --dataset_names "ARC-Easy" \
    --steering_methods "no_steering" "optimal_probe" "optimal_logistic_probe" "optimal_contrastive" "additive_probe" "additive_logistic_probe" "vanilla_contrastive" "prompt_steering" \
